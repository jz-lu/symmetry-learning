...
parser.add_argument("L", type=int, help='number of qubits')
...
np.save(OUTDIR + f'losses_{NUM_QUBITS}_{STATE_TYPE}.npy', losses)
np.save(OUTDIR + f'queries_{NUM_QUBITS}_{STATE_TYPE}.npy', queries)

# # Plot the scaling complexity
# avgs = np.mean(queries, axis=1)
# stdevs = np.std(queries, axis=1)
# x = np.arange(args.Lmin, NUM_QUBITS+1)
# COLOR = 'darkblue'
