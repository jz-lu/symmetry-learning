...
class SymFinder():
  def __init__(self, lr1, gradient_step1, lr2, gradient_step2):
    ...
    self.lr1 = lr1 # this is the learning rate for updating parameter when sym finding
    self.lr2 = lr2 #this is the lr for exploring
    self.gradient_step1 = gradient_step1
    ...
    self.KS_threshold = 0.0005 #if less than value, declare it as symmetry
    #self.model2 = KS_net()
  ...
  def update_param_true_loss(self):
    ...
    for i in range(3):
        for j in range(3):
            ...
            new_param[i,j] = new_param[i,j] + self.gradient_step1
            new_param = torch.fmod(new_param, 1000*pi)
            new_state = self.transform(new_param)
            new_loss = self.calculate_loss(new_state, new_param)[1]
            grad[i,j] = (new_loss - cur_loss) / self.gradient_step1
    #update the parameters:
    self.parameters = torch.fmod(self.parameters - self.lr1 * grad, 2*pi)
  ...
  def symfinding(self):
    ...
    for i in range(200):
      cur_true_loss = self.update_param_true_loss()
...
finder = SymFinder(0.01, 0.01, 0.03, 0)
